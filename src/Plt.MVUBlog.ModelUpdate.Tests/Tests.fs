  ...
  testList "MVULibrary" [
    testList "Math" [
      testCase "Test Primes of 521 (only 1)" <| fun _ ->
        let x = primeFactors 521
        Expect.equal x [521] "Prime of 521 is 521"
      
      testCase "Test Primes 28 (2)" <| fun _ ->
          let x = primeFactors 28
          Expect.equal x [7;2;2] "Primes of 28 are 4,7"      
    ]
    testList "ModelUpdate" [
      testProperty "Plus/Minus Increment" <| fun a ->
        ...
        let res = if a >= 0 then [0..(a-1)] else [(a+1)..0] 
                  |> List.fold (fun model _ -> update (Operation (operator,1)) model) baseRecord
        res.Value = a
